...
const Updatesmart = () => {
 ...
 useEffect(()=>{
      fetch(`http://localhost:7003/updatesmart`)
      .then((res)=>res.json())
      .then((data)=>
   ...
   const DeleteItems=(id)=>{
    fetch(`http://localhost:7003/deletesmart/${id}`,{
      method:"DELETE",})
      .then((res)=>res.json())
      .then((data)=>{
          ...
          {ProductItems?.map((Item) => (
            <tr key={Item._id}>
              ...
              <td>
                ...
                <Link to={`/editsmart/${Item._id}`}><button><FiEdit /></button></Link>
              </td>
            </tr>
          ))}
        </tbody>
